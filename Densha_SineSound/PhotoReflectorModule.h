...
private:
  ...
  static const uint16_t historyLength = 30;  //ここでオン・オフともに差分をためる分の長さ（30→10で高速化）
  int32_t history[historyLength] = { 0 };
  uint32_t historyIndex = 0;
  int32_t sensorSum = 0;  //センサーオフのときの足し算したやつ
  int32_t current = 0;
  static const int32_t thresholdOnce = 6;  //ここで感度変えられそう(-分の差分)
  static const int32_t threshold = thresholdOnce * historyLength;
  bool pastSensorFlag = false;
  bool triggered = false;

public:
  static const uint32_t sensorLatency = 20;  // センサーの応答特性（micro sec）20→10で高速化
};
